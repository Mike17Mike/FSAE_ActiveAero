...
//Needed Variables 
int brakePercent;              //Variable for brake percentage
...
int main() {
    ...
    while(1) {
        if(can1.read(msg)){         //Gets a message from the can bus
            ...
            if(msg.id == 0){                                //Check to see if you need to use button or if aero is automatic
                automaticAero = msg.data[2];                //The data to know if the active aero is manual or auto
            }

            if(msg.id == 0 && automaticAero == 1){          //Check button for manual aero
...
void moveAero(int x){                                        //Moves aero wing to given input
